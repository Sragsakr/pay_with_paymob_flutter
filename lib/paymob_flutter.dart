...
class PaymobFlutter implements PaymentServiceInterface {
  ...
  Future<bool> initialize({
    ...
    throw const PaymentInitializationException(
        'Please use initializeWithConfig() method with publicKey and secretKey for Intention Api');
  }
  ...
  void _validateInitialization() {
    ...
    _validateIntentionApiKeys();
  }

  /// Validate that the service has required keys for Intention Api
  void _validateIntentionApiKeys() {
    if (_publicKey == null || _publicKey!.isEmpty) {
      throw const InvalidApiKeyException(
          'Public key is required for Intention Api');
    }
    if (_secretKey == null || _secretKey!.isEmpty) {
      throw const InvalidApiKeyException(
          'Secret key is required for Intention Api');
    }
  }
